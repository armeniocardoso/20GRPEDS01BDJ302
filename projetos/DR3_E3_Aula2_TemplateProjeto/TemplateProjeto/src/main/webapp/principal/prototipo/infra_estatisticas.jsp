<%@page contentType="text/html" pageEncoding="utf-8"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
        <title>Sistema Integrado de Gestão</title>
        <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/estilos.css" media="screen" />
        <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/jquery-ui-1.8.16.custom.css" media="screen" />
    </head>
    <body>
        <center>
            <br />
            <h3>Protótipo do SIGE</h3>
            <div class="mensagemInformacao" style="width:550px">
                <p><strong>Objetivo desta Tela:</strong></p>
                <p>
                    Aqui os GESTORES acessam as informações gerenciais na forma de
                    gráficos e tabelas.
                </p>
                <p>
                    O objetivo é responder algumas perguntas gerenciais sobre a Infra:
                </p>
                <p>
                    Tipos de Processadores, Sistemas Operacionais etc.
                </p>
                <p>
                    Inicialmente essa tela exibirá gráficos de torta, confrontando 
                    os números de cada item.
                </p>
                <p>
                    <a href="http://www.jfree.org/jfreechart/samples.html" target="_blank">Exemplos</a>
                </p>
            </div>               
        </center>        
    </body>
</html>
